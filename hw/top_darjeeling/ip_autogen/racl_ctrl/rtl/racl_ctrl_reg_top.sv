...
module racl_ctrl_reg_top
  ...
  always_comb begin
    ...
    unique case (reg_addr)
      ...
      RACL_CTRL_POLICY_ALL_RD_WR_SHADOWED_OFFSET: begin addr_valid = 1; addr_idx = 0; end
      RACL_CTRL_POLICY_ROT_PRIVATE_SHADOWED_OFFSET: begin addr_valid = 1; addr_idx = 1; end
      RACL_CTRL_POLICY_SOC_ROT_SHADOWED_OFFSET: begin addr_valid = 1; addr_idx = 2; end
      RACL_CTRL_INTR_STATE_OFFSET: begin addr_valid = 1; addr_idx = 3; end
      RACL_CTRL_INTR_ENABLE_OFFSET: begin addr_valid = 1; addr_idx = 4; end
      RACL_CTRL_INTR_TEST_OFFSET: begin addr_valid = 1; addr_idx = 5; end
      RACL_CTRL_ALERT_TEST_OFFSET: begin addr_valid = 1; addr_idx = 6; end
      RACL_CTRL_ERROR_LOG_OFFSET: begin addr_valid = 1; addr_idx = 7; end
      RACL_CTRL_ERROR_LOG_ADDRESS_OFFSET: begin addr_valid = 1; addr_idx = 8; end
  ...
  always_comb begin
    ...
    if (reg_we && racl_addr_write_valid) begin
      case (racl_addr_write_idx)
        // TODO: use the register index enum entries instead?
        0: wr_err = |(RACL_CTRL_PERMIT[0] & ~reg_be);
        1: wr_err = |(RACL_CTRL_PERMIT[1] & ~reg_be);
        2: wr_err = |(RACL_CTRL_PERMIT[2] & ~reg_be);
        3: wr_err = |(RACL_CTRL_PERMIT[3] & ~reg_be);
        4: wr_err = |(RACL_CTRL_PERMIT[4] & ~reg_be);
        5: wr_err = |(RACL_CTRL_PERMIT[5] & ~reg_be);
        6: wr_err = |(RACL_CTRL_PERMIT[6] & ~reg_be);
        7: wr_err = |(RACL_CTRL_PERMIT[7] & ~reg_be);
        8: wr_err = |(RACL_CTRL_PERMIT[8] & ~reg_be);
      endcase
    end
  end

  // Generate write-enables
  assign policy_all_rd_wr_shadowed_re = racl_addr_read_valid & (racl_addr_read_idx == 0) & reg_re & !reg_error;
  assign policy_all_rd_wr_shadowed_we = racl_addr_write_valid & (racl_addr_write_idx == 0) & reg_we & !reg_error;
  ...
  assign policy_rot_private_shadowed_re = racl_addr_read_valid & (racl_addr_read_idx == 1) & reg_re & !reg_error;
  assign policy_rot_private_shadowed_we = racl_addr_write_valid & (racl_addr_write_idx == 1) & reg_we & !reg_error;
  ...
  assign policy_soc_rot_shadowed_re = racl_addr_read_valid & (racl_addr_read_idx == 2) & reg_re & !reg_error;
  assign policy_soc_rot_shadowed_we = racl_addr_write_valid & (racl_addr_write_idx == 2) & reg_we & !reg_error;

  assign policy_soc_rot_shadowed_read_perm_wd = reg_wdata[15:0];
  assign policy_soc_rot_shadowed_write_perm_wd = reg_wdata[31:16];


  assign intr_enable_we = racl_addr_write_valid & (racl_addr_write_idx == 4) & reg_we & !reg_error;

  assign intr_enable_wd = reg_wdata[0];

  assign intr_test_we = racl_addr_write_valid & (racl_addr_write_idx == 5) & reg_we & !reg_error;

  assign intr_test_wd = reg_wdata[0];

  assign alert_test_we = racl_addr_write_valid & (racl_addr_write_idx == 6) & reg_we & !reg_error;

  assign alert_test_fatal_fault_wd = reg_wdata[0];
  assign alert_test_recov_ctrl_update_err_wd = reg_wdata[1];

  assign error_log_we = racl_addr_write_valid & (racl_addr_write_idx == 7) & reg_we & !reg_error;
