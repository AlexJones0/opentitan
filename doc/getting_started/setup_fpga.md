...
Instead, the bazel-build ROMs are programmed to the relevant FPGA BRAMs via the bkdr_loader, using the mechanism described [here](#programming-the-fpga-with-rom--otp-images).
...
From this point onwards, you cannot use `opentitantool fpga backdoor ...` without first calling `opentitantool fpga backdoor enter`, which will reset the chip.
...
3. Run `opentitantool`.
   ...
   bazel build //sw/device/examples/hello_world:hello_world_fpga_${INTERFACE}_bin
