...
/// bitfields to use for transmission. Does not support SPI device encoding.
    ...
    SpiBitbangEncoder<D0, D1, D2, D3, CLK, CS>
{
    ...
    fn encode_word(
        ...
        words: &[u8],
        samples: &mut Vec<u8>,
    ) -> Result<bool, SpiTransferEncodeError> {
        if self.first_word && !words.is_empty() {
            if !self.cs_asserted {
        ...
        while encoded_bits < self.config.bits_per_word {
            if encoded_bits % 8 == 0 {
                let Some(&next_byte) = words.next() else {
                    if encoded_bits == 0 {
                        return Ok(false); // Do not encode an empty last word
                    }
                    break;
                };
                byte = next_byte;
    ...
    fn encode_words(
        ...
        for word in words.chunks(bytes_per_word) {
            ...
            if !self.encode_word(word, samples)? {
                break;
            }
        }
        Ok(())
    ...
    /// `assert_cs` should be called appropriately to ensure that CS is asserted before
    ...
    /// asserting and later de-asserting the CS as required.
...
mod test {
    ...
    fn encoding_delays() -> Result<()> {
        ...
        for ((inter_word_delay, cs_hold_delay, cs_release_delay), expected) in tests {
            ...
            for cpha in [false, true] {
                ...
                // Run an encode/decode test with these delays.
    ...
    fn encode_host_reads() -> Result<()> {
        ...
        expected.append(&mut [1, 0].repeat(8 * 5));
        expected.push(2);
        assert_eq!(samples, expected);
        Ok(())
    }
