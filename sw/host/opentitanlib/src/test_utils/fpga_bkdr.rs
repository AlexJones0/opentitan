...
use crate::io::fpga_bkdr::{Backdoor, BackdoorParams, BackdoorTarget, enter_backdoor_loader};
...
pub fn enter_bkdr<'a>(
    ...
    let bkdr_params = BackdoorParams {
        jtag: jtag_params.clone(),
    };
    bkdr_params.create(transport)?.connect(true)
}
...
impl TargetWrite {
    ...
    pub fn bkdr_write(&self, bkdr: &mut Backdoor, verify: bool) -> Result<()> {
        let mut target = bkdr
...
impl LoadMemories {
    pub fn init(&self, transport: &TransportWrapper, jtag_params: &JtagParams) -> Result<()> {
        let mut bkdr = enter_bkdr(transport, jtag_params)?;
        self.targets
            .iter()
            .try_for_each(|t| t.bkdr_write(&mut bkdr, false))?;
        bkdr.set_done()?;
