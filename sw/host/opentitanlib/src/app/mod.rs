    ...
    fn resolve_uart_conf(
        ...
        if let Some(entry) = uart_conf_map.get(name.to_uppercase().as_str()) {
            ...
            if let Some(parity) = &entry.parity {
                conf.parity = Some(parity.clone());
            }
            if let Some(stop_bits) = &entry.stopbits {
                conf.stop_bits = Some(stop_bits.clone());
            }
            Ok(conf)
        } else {
...
impl TransportWrapper {
    ...
    pub fn spi(&self, name: &str) -> Result<Rc<dyn Target>> {
        ...
        if self.wrap_bitbangs.get() {
            ...
            // TODO: do we need an extra cache in BitbangWrapper?
            // TODO: how do I handle the case where we have an existing instance
            //       that e.g. has chip select asserted?
    ...
    pub fn uart(&self, name: &str) -> Result<Rc<dyn Uart>> {
        ...
        if let Some(conf) = uart_conf {
            if let Some(baud_rate) = conf.baud_rate {
                ...
                let _ = uart.set_baudrate(baud_rate).is_err();
            }
            if let Some(parity) = &conf.parity {
                // Ignore errors as some transports don't let you set parity
                let _ = uart.set_parity(match *parity {
