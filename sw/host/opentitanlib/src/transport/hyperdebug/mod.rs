...
impl Conn {
    ...
    fn find_hyperdebug_console_lines(
        ...
        for i in 0..*len {
            if buf[i] == b'\n' {
                let mut line_end = i;
                while line_end > line_start && buf[line_end - 1] == 0x0D {
    ...
    fn execute_command(&self, cmd: &str, mut callback: impl FnMut(&str)) -> Result<()> {
        ...
        // larger than 58 bytes (we give extra room for echoing LF/CR, '> ' prompts,
        // and CTRL-c), we split the command into chunks and wait for each individual
        // chunk to be echoed before proceeding.
        let cmd = format!("{}\n", cmd);
        let mut cmd_chunks = cmd.as_bytes().chunks(58);
        ...
        for _ in 1..cmd_chunks.len() {
            ...
            while !echo_seen {
                ...
                if response.len() >= cmd_chunk.len() {
                    if response[response.len() - cmd_chunk.len()..] == *cmd_chunk {
                        echo_seen = true;
                        ...
                        log::info!("Unexpected output: {:?}", response);
                    }
                }
            }
        }
