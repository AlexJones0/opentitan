...
use opentitanlib::crypto::spx::SpxKeyFormat;
use sphincsplus::{SphincsPlus, SpxRawSignature, SpxSignatureMode, SpxSecretKey};
...
impl CommandDispatch for SpxKeyShowCommand {
    fn run(
        ...
        let key = spx::load_public_key(&self.key_file)?;
...
pub struct SpxKeyGenerateCommand {
    ...
    /// Key encoding format (pre-standard, pkcs8-pem, pkcs8-der).
...
pub struct SpxKeyConvertCommand {
    /// Key encoding format to convert to (pre-standard, pkcs8-pem, pkcs8-der).
    #[arg(long, default_value_t = SpxKeyFormat::default())]
    format: SpxKeyFormat,
...
pub struct SpxVerifyCommand {
    ...
    /// The file containing the SPHINCS+ public key.
    #[arg(value_name = "KEY")]
    public_key: PathBuf,
