...
static void init_peripherals(void) {
  ...
  dif_result_t result = dif_otp_ctrl_configure(&otp, config);
  CHECK(result != kDifLocked, "OTP DIF is locked");
  // Rstmgr
...
static void keymgr_advance_to_creator_root_key(bool expect_init_fail) {
  ...
  if (is_using_test_rom) {
    ...
    LOG_INFO("Using rom, only advancing state... %d", expect_init_fail);
...
bool test_main(void) {
  ...
  LOG_INFO("initialized peripherals");
