...
def _local_sign(ctx, tool, digest, ecdsa_key, rsa_key, spxmsg = None, spx_key = None, profile = None):
    ...
    print("local {} sign".format(key_command))
    ...
    if spxmsg and spx_key:
        ...
        legacy = _get_bool(spx_key.config, "legacy", "false")
        alg = "spx" if legacy else "slh-dsa"
        print("local {} sign, domain={}, spx_sig.path={}, spxmsg.path={}, private_key.path={}".format(alg, domain, spx_sig.path, spxmsg.path, private_key.path))
